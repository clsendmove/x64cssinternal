...
// credits to RileyZT (otherwise known as twotrick)'s cutebase.
// i had to fix it due to how poorly its coded. (or he forgor, i dunno)
// nigga was feeding 0 TO THE FONT SIZE on text.. and calculating the text width wrong.
// alot of other stuff was wrong, so this should be much better.
...
void c_drawsystem::initialize(IDirect3DDevice9* device, HWND window) {
    ...
    ImFontConfig segoe_cfg;
    ImFontConfig tahoma_cfg;
    ImFontConfig verdana_cfg;

    verdana_cfg.MergeMode = false;
    segoe_cfg.MergeMode = false;
    tahoma_cfg.MergeMode = false;

    static const ImWchar ranges[] = { 0x0020, 0xFFFF, 0 }; // this does work but it doesnt display some unicodes.
    

    // https://www.dafont.com/small-pixel.font
    /*
    fonts::FONT_SMALL = io.Fonts->AddFontFromFileTTF(
        R"(C:\Users\gigabyte\AppData\Local\Microsoft\Windows\Fonts\small_pixel.ttf)",
        8.f, nullptr, nullptr
    );*/

    fonts::FONT_SEGOE = io.Fonts->AddFontFromFileTTF(
        R"(C:\Windows\Fonts\SegoeUI.ttf)",
        16.f, &segoe_cfg, ranges
    );

    /*
    fonts::FONT_TAHOMA = io.Fonts->AddFontFromFileTTF(
        R"(C:\Windows\Fonts\tahoma.ttf)",
        13.f, &tahoma_cfg, ranges
    );

    fonts::FONT_VERDANA = io.Fonts->AddFontFromFileTTF(
        R"(C:\Windows\Fonts\verdana.ttf)",
        12.f, &tahoma_cfg, ranges
    );*/
...
void c_render::string(ImDrawList* list, ImFont* font, int x, int y, bool outlined, e_text_align align,
    ...
    if (input.empty()) // we have no input, don't even bother trying
        return;
    if (font == nullptr) // moron..
        return;
    // Get the text size... CORRECTLY. this time.
    ImVec2 size = font->CalcTextSizeA(font->LegacySize, FLT_MAX, 0.0f, input.data(), nullptr, nullptr);
    float  left, top;

    // this retarded. why are we casting int to float? 
    // its common sense that the values are going to be something
    // like: int -> 1, float 1.0f.. whats even the point?
    ...
    if (outlined) {
        // Because ImGui has no native method of doing outlines, we must draw the same text
        ...
        left = f_x;
        top = f_y + 1.f;
        list->AddText(font, font->LegacySize, { left, top }, ImColor(0.f, 0.f, 0.f, col.Value.w), input.data(), nullptr);
        left = f_x + 1.f;
        top = f_y;
        list->AddText(font, font->LegacySize, { left, top }, ImColor(0.f, 0.f, 0.f, col.Value.w), input.data(), nullptr);
        left = f_x - 1.f;
    ...
    left = f_x;
    top = f_y;
    list->AddText(font, font->LegacySize, { left, top }, col, input.data(), nullptr);
}
...
void c_render::filled_box(ImDrawList* list, int x, int y, int w, int h, ImColor col) {
    ...
    list->AddRectFilled({ f_x, f_y }, { f_x + f_w, f_y + f_h }, col);
}
...
void c_render_threaded::paint_traverse(ImDrawList* list) {
    ...
    for (auto&& cmds : m_safe_cmds) {
        ...
        if (auto* string = std::get_if<string_t>(&cmds)) {
            u::draw_add.string(list, string->font, string->x, string->y, string->outlined, string->align,
                string->col, string->input);
        }
        else if (auto* box = std::get_if<box_t>(&cmds)) {
            u::draw_add.box(list, box->x, box->y, box->width, box->height, box->outlined, box->col);
        }
        else if (auto* circle = std::get_if<circle_t>(&cmds)) {
            u::draw_add.circle(list, circle->x, circle->y, circle->rad, circle->outlined, circle->col);
        }
        else if (auto* filled = std::get_if<filled_box_t>(&cmds)) {
            u::draw_add.filled_box(list, filled->x, filled->y, filled->width, filled->height, filled->col);
        }
        else if (auto* line = std::get_if<line_t>(&cmds)) {
            u::draw_add.line(list, line->x1, line->y1, line->x2, line->y2, line->col);
        }
    }
}
