...
public:
    ...
    virtual vec3 const& get_render_angles(void) = 0;
    ...
    virtual const DWORD* get_model() const = 0;
    virtual int                   draw_model(int flags) = 0;
    void draw_model_(int flags) {
        typedef void(__fastcall* OriginalFn)(void*, int);
        return memory::get_vfunc<OriginalFn>(this, 10)(this, flags);
    }
    ...
    virtual void get_shadow_render_bounds(vec3& mins, vec3& maxs, e_shadow_type shadow_type) = 0;
    ...
    virtual bool get_shadow_cast_direction(vec3* direction, e_shadow_type shadow_type) const = 0;
    ...
    virtual bool                  get_attachment(int number, vec3& origin, vec3& angles) = 0;
